...
* **`client/`** - Angular Frontend Application
    ...
    * `node_modules/` - Third-party Node.js libraries.
    * `src/` - Source code for the Angular application.
        * `app/` - Core application logic.
            * `auth/` - Authentication module, containing:
                * `login/` - Login component.
                * `sso-callback/` - Component for handling SSO redirections.
                * `auth-routing.module.ts` - Routing for the authentication module.
                * `auth.module.ts` - Authentication module definition.
                * `auth.service.ts` - Service for authentication (login, SSO).
    ...
    * `proxy.conf.json` - Proxy configuration for API calls during development.

* **`server/`** - Spring Boot Backend Application
    * `src/` - Source code for the Spring Boot application.
        * `main/`
            * `java/` - Java source files.
                * `com/nveapp/` - Main package for the application.
                    * `AppAplication.java` - Main Spring Boot entry point.
                    * `controller/` - REST API controllers (e.g., `AuthController`, `SecureController`).
                    * `dto/` - Data Transfer Objects for API requests/responses (e.g., `LoginRequest`, `SsoCallbackResponse`).
                    * `exception/` - Custom exceptions for error handling.
                    * `model/` - JPA entities or domain models (e.g., `User`).
                    * `repository/` - Data access layer interfaces and implementations.
                    * `security/` - Spring Security configurations, JWT filter, and utility.
                    * `service/` - Business logic services (e.g., `AuthService`, `SsoService`).
            * `resources/` - Application resources (e.g., `application.properties` for configuration).
    * `target/` - Directory for compiled classes and build artifacts (automatically generated by Maven, ignored by Git).
